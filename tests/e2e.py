...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="vsp_mcp_e2e_") as tmp:
        ...
        check("two components created", len(geoms) == 2, str([g["name"] for g in geoms]))
        ...
        check(
            "sections listed individually",
            {"XSec_0", "XSec_1"} <= set(listing["groups"]),
            str([g for g in listing["groups"] if g.startswith("XSec")]),
        )
        span = next(
            p
            for p in c.tool("vsp_list_parms", {"geom_id": wing["id"], "group": "XSec_1"})["parms"]
            if p["name"] == "Span"
        )
        applied = c.tool(
            "vsp_set_parms",
            {
                "geom_id": wing["id"],
                "parms": [{"group": span["group"], "name": "Span", "value": 8.0}],
            },
        )
        ...
        check(
            "STEP written",
            c.tool("vsp_export", {"path": out("m.step"), "export_format": "STEP"})["ok"],
        )
        ...
        check(
            "surfaces split by type",
            split["thin"] == ["MainWing"] and split["thick"] == ["Body"],
            str(split),
        )
        polar = sweep["polar"]
        check("polar has one point per alpha", len(polar["Alpha"]) == 4, str(polar["Alpha"]))
        check(
            "lift rises with alpha",
        ...
        check(
            "api script runs",
            c.tool(
                "vsp_run_api_script", {"code": "result = vsp.GetGeomTypeName(vsp.FindGeoms()[0])"}
            )["result"]
            == "Wing",
        )
        try:
            c.tool(
                "vsp_get_parms",
                {"geom_id": wing["id"], "parms": [{"group": "Nope", "name": "Nope"}]},
            )
