...
GEOM_TYPES = ["POD", "FUSELAGE", "WING", "STACK", "BLANK", "ELLIPSOID",
              "BODYOFREVOLUTION", "HUMAN", "PROP", "GEAR", "HINGE",
              "CONFORMAL", "ROUTING", "AUXILIARY", "COBRA"]

EXPORT_FORMATS = ["STL", "STEP", "IGES", "OBJ", "X3D", "DXF", "SVG", "GMSH",
                  "VSPGEOM", "CART3D", "NASCART", "POVRAY", "BEM",
                  "SELIG_AIRFOIL", "BEZIER_AIRFOIL"]

# Analyses cheap and safe enough to actually execute on a plain wing.
RUNNABLE = ["MassProp", "CompGeom", "DegenGeom", "Projection", "PlanarSlice",
...
def main():
    ...
    c.tool("vsp_new_model"); covered.add("vsp_new_model")
    bad_types = []
    ...
    c.tool("vsp_delete_geom", {"geom_id": geoms[-1]["id"]}); covered.add("vsp_delete_geom")
    ok("delete removes one", len(c.tool("vsp_list_geoms")) == len(GEOM_TYPES))

    sets = c.tool("vsp_list_sets"); covered.add("vsp_list_sets")
    ok("built-in sets present",
       [s["name"] for s in sets[:3]] == ["All", "Shown", "Not_Shown"])
    wing = next(g for g in c.tool("vsp_list_geoms") if g["type"] == "Wing")
    r = c.tool("vsp_assign_set", {"geom_id": wing["id"], "set_index": 4,
                                  "member": True, "set_name": "MyLifting"})
    ...
    ok("remove from user set",
       c.tool("vsp_assign_set", {"geom_id": wing["id"], "set_index": 4,
                                 "member": False})["geoms"] == [])

    before = [(g["name"], g["type"]) for g in c.tool("vsp_list_geoms")]
    c.tool("vsp_save_model", {"path": out("rt.vsp3")}); covered.add("vsp_save_model")
    c.tool("vsp_new_model")
    ok("new model clears", c.tool("vsp_list_geoms") == [])
    c.tool("vsp_open_model", {"path": out("rt.vsp3")}); covered.add("vsp_open_model")
    after = [(g["name"], g["type"]) for g in c.tool("vsp_list_geoms")]
    ok("save/open round-trips the model", before == after,
       f"{len(before)} components")
    ...
    ok("analyses leave no mesh residue",
       [g["name"] for g in c.tool("vsp_list_geoms")] == ["W"],
       str([g["name"] for g in c.tool("vsp_list_geoms")]))

    print("\n[6] convenience wrappers and info")
    covered |= {"vsp_mass_properties", "vsp_comp_geom", "vsp_info",
                "vsp_list_parms", "vsp_get_parms", "vsp_set_parms",
                "vsp_run_api_script", "vsp_vspaero_sweep"}
    info = c.tool("vsp_info")
    ok("info reports version and vspaero", info["version"].startswith("OpenVSP")
       and bool(info["vspaero_path"]))
    mp = c.tool("vsp_mass_properties")["results"]
    cg = c.tool("vsp_comp_geom")["results"]
    ok("mass and wetted area positive",
    ...
    bad = [
        ...
        ("unknown export format", "vsp_export",
         {"path": out("x.zzz"), "export_format": "NOPE"}),
    ...
    ok("server still healthy afterwards",
       c.tool("vsp_info")["version"].startswith("OpenVSP"))
    ok("model unharmed by the failures",
       [g["name"] for g in c.tool("vsp_list_geoms")] == ["W"])

    print("\n[8] concurrency — API is not thread-safe, lock must serialise")
    reqs = [("vsp_mass_properties", {}), ("vsp_comp_geom", {}),
            ("vsp_info", {}), ("vsp_list_geoms", {}),
            ("vsp_mass_properties", {}), ("vsp_comp_geom", {})]
    replies = c.pipeline(reqs)
    ok("all overlapping requests answered", len(replies) == len(reqs))
    ok("none returned an error",
       not any(r.get("result", {}).get("isError") for r in replies))
